...
df_variants = (ds_logreg[['variants', 'variant_position', 'variant_quality', 'pvalue', 'p_values.adj', 'mutation', 'odds.ratio', 'effect', 'gene.name', 'gene.product', 'gene.type']]
            .to_pandas()
...
df_allele_name = ds_logreg.variant_allele.to_pandas().drop([2,3], axis=1).rename(columns={0: 'ref', 1: 'alt'})
df_allele_count = ds_logreg.pipe(sg.variant_stats).variant_allele_count.to_pandas().drop([2,3], axis=1).rename(columns={0: 'ref_count', 1: 'alt_count'})
