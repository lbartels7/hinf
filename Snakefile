...
rule all:
    input:  outdir+"/samples_AMP_nonNAN.csv",
            outdir+"/stats.tsv",
...
rule extract_AMP_nonNAN:
    ...
    output: outdir+"/samples_AMP_MIC_nonNAN.csv",
...
rule vcf_to_vcfgz:
    ...
    output: temp(outdir+"/vcf/{id}.gatk.vcf.gz"),
...
rule normalize:
    input: outdir+"/Hinf.vcf.gz"
    output: outdir+"/Hinf_norm.vcf.gz"
    # conda: "envs/bcftools.yaml"
...
rule filter_samples_whole_genome:
    input: outdir+"/Hinf_norm.vcf.gz",
            outdir+"/samples_AMP_MIC_nonNAN.csv",
            outdir+"/samples_AMP_nonNAN.csv"
    output: outdir+"/Hinf_norm_mic.vcf.gz",
...
rule filter_samples_genes:
    input: outdir+"/Hinf_norm_genes.vcf.gz",
            outdir+"/samples_AMP_MIC_nonNAN.csv",
            outdir+"/samples_AMP_nonNAN.csv"
    output: outdir+"/Hinf_norm_genes_mic.vcf.gz",
...
rule create_zarrs:
    input: outdir+"/{name}.vcf.gz"
    output: directory(outdir+"/zarrs/{name}.zarr")
    conda: "envs/sgkit.yaml"
    script: "scripts/create_zarrs.py" 


rule filter_heterozygous_calls_and_map:
    input: outdir+"/zarrs/{name}.zarr",
            "vcf_Haemophilus/annotation_files/metadata_HLR_extern.xlsx"
    output: directory(outdir+"/zarrs/{name}_temp.zarr"),
...
rule zarr_to_vcf:
    input: outdir+"/zarrs/Hinf_norm_temp.zarr"
    output: outdir+"/Hinf_norm_temp.vcf" # set temp
    run:
        import sgkit as sg
...
rule linear_regression:
    input: outdir+"/feather/Hinf_norm_mic.feather",
            "vcf_Haemophilus/annotation_files/Hinf_Rd-KW20v3_DSM11121_2023-06-15_genes_adjst.txt"
    output: outdir+"/regression/linear_regression_unscaled.csv"
...
rule linear_regression_logscaled:
    input: outdir+"/feather/Hinf_norm_mic.feather",
            "vcf_Haemophilus/annotation_files/Hinf_Rd-KW20v3_DSM11121_2023-06-15_genes_adjst.txt"
    output: outdir+"/regression/linear_regression_logscaled.csv"
    conda: "envs/regression.yaml"
...
rule linear_regression_rankscaled:
    input: outdir+"/feather/Hinf_norm_mic.feather",
            "vcf_Haemophilus/annotation_files/Hinf_Rd-KW20v3_DSM11121_2023-06-15_genes_adjst.txt"
    output: outdir+"/regression/linear_regression_rankscaled.csv"
    conda: "envs/regression.yaml"
    script: "scripts/linear_regression_rankscaled.R"

rule logistic_regression:
    input: outdir+"/feather/Hinf_norm_bin.feather",
            "vcf_Haemophilus/annotation_files/Hinf_Rd-KW20v3_DSM11121_2023-06-15_genes_adjst.txt"
    output: outdir+"/regression/logistic_regression.csv"
    conda: "envs/regression.yaml"
...
rule create_final_logistic_regression_result_object:
    ...
    output: directory(outdir+"/zarrs/Hinf_norm_bin_logreg_results.zarr")
    conda: "envs/sgkit.yaml"
    script: "scripts/annotate_with_mutations.py" 


rule create_results_table_linreg:
    input: outdir+"/zarrs/Hinf_norm_mic_linreg{scaling}_results.zarr"
    output: outdir+"/linreg{scaling}.csv"
    conda: "envs/sgkit.yaml"
    script: "scripts/create_results_table_linreg.py"

rule create_results_table_logreg:
    input: outdir+"/zarrs/Hinf_norm_bin_logreg_results.zarr"
    output: outdir+"/logreg.csv"
    conda: "envs/sgkit.yaml"
    script: "scripts/create_results_table_logreg.py"


rule extract_most_significant_snps_linreg:
    input: outdir+"/linreg{scaling}.csv"
    output: outdir+"/sigsnps_linreg{scaling}.txt"
...
rule extract_most_significant_snps_logreg:
    input: outdir+"/logreg.csv"
    output: outdir+"/sigsnps_logreg.txt"
